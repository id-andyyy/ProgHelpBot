...
@router.message(Command(commands=['addarticle']), StateFilter(default_state))
...
@router.message(StateFilter(FSMAddArticle.fill_title))
...
@router.message(StateFilter(FSMAddArticle.fill_emoji))
...
@router.message(StateFilter(FSMAddArticle.fill_link))
...
@router.message(StateFilter(FSMAddArticle.fill_keywords))
...
@router.message(StateFilter(FSMAddArticle.fill_section))
...
@router.message(StateFilter(FSMAddArticle.fill_position))
async def process_position_sent(message: Message, state: FSMContext):
...
@router.message(StateFilter(FSMAddArticle.fill_is_available),
                Text(text=[LEXICON_KEYBOARDS_ADMIN['available_button'],
                           LEXICON_KEYBOARDS_ADMIN['not_available_button']]))
async def process_is_available_sent(message: Message, state: FSMContext):
    ...
    await message.answer(text=f'<b>Название:</b> {user_dict[message.from_user.id]["title"]}\n'
                         ...
                         reply_markup=create_allow_publishing_keyboard())  # add keyboard
...
@router.message(StateFilter(FSMAddArticle.allow_publishing),
                Text(text=[LEXICON_KEYBOARDS_ADMIN['allow_publishing_button'],
                           LEXICON_KEYBOARDS_ADMIN['not_allow_publishing_button']]))
